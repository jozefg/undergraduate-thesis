...
Girard's method~\citep{Girard:71,Girard:72}, see \citet{Harper:16} for
...
recurring theme of Kripke logical relations, or sheaf models, is to
abandon the notion of a single global truth and judge truth relative
to a current state of the world. Rather than considering
$(e_1, e_2) \in \den{\tau}_\eta$, we should consider at some world
$w$: $(e_1, e_2) \den{\tau}_\eta(w)$. Now the collection of worlds
should be \emph{ordered} by a reachability relation
$\reach$. Importantly, Kripke logical relations should be monotone in
relation to $\reach$:
\[
  \forall w_1, w_2.
...
\citet{Pitts:98}, means that our Kripke worlds should
...
logics~\citep{Dreyer:09,Krogh-Jespersen:17}.
...
\citet{Smyth:77} and subsumed prior work by Scott
constructing recursive domain equations by
hand~\citep{Scott:76}. One potential issue is that we would like to
...
Rutten~\citep{America:87}. The precise accounting the theorem can
be found in \citet{Birkedal:steps:11} allowing for any enriched functor
